...
type elf32_address=dword;
     ...
     {ELF File}
     elf_section_header_ptr=packed record
                            case Boolean of
                            True:(sec32ptr:Pelf32_section_header;);
                            False:(sec64ptr:Pelf64_section_header;);
                            end;
     elf_program_header_ptr=packed record
                            case Boolean of
                            True:(pro32ptr:Pelf32_program_header;);
                            False:(pro64ptr:Pelf64_program_header;);
                            end;
     elf_rel_ptr=packed record
                 case Boolean of
                 True:(rel32ptr:Pelf32_rel;);
                 False:(rel64ptr:Pelf64_rel;);
                 end;
     elf_rela_ptr=packed record
                  case Boolean of
                  True:(rela32ptr:Pelf32_rela;);
                  False:(rela64ptr:Pelf64_rela;);
                  end;
     elf_dynamic_ptr=packed record
                     case Boolean of
                     True:(dyn32:Pelf32_dynamic_entry;);
                     False:(dyn64:Pelf64_dynamic_entry;);
                     end;
     elf_symbol_ptr=packed record
                    case Boolean of
                    True:(sym32:Pelf32_symbol_table_entry;);
                    False:(sym64:Pelf64_symbol_table_entry;);
                    end;
     elf_header_ptr=packed record
                    case Boolean of
                    True:(hdr32:Pelf32_header;);
                    False:(hdr64:Pelf64_header;);
                    end;
     elf_content_ptr=packed record
                     case Byte of
                     0:(b:Pbyte;);
                     1:(w:Pword;);
                     2:(d:Pdword;);
...
const elf_magic:array[1..4] of char=(#$7F,'E','L','F');
      ...
      elf_abi_version_position:byte=8;
      ...
      {ELF Machine Value}
      elf_machine_none:word=0;
      elf_machine_386:word=3;
      elf_machine_arm:word=40;
      elf_machine_x86_64:word=62;
      elf_machine_aarch64:word=183;
      elf_machine_riscv:word=243;
      elf_machine_loongarch:word=258;
      {ELF Version Value}
...
function elf_symbol_type_info(val:byte):byte;
...
function elf_symbol_type_bind(val:byte):byte;
begin
 elf_symbol_type_bind:=val shr 4;
end;
function elf_symbol_type_type(val:byte):byte;
begin
 elf_symbol_type_type:=val and $F;
end;
function elf_symbol_type_info(val:byte):byte;
begin
 elf_symbol_type_info:=val shr 4 shl 4+val and $F;
end;
function elf_symbol_type_visibility(other:byte):byte;
begin
...
function elf32_reloc_info(val:elf32_word):elf32_word;
begin
 elf32_reloc_info:=val shr 8 shl 8+val and $FF;
end;
function elf64_reloc_symbol(val:elf64_xword):elf64_xword;
begin
...
function elf64_reloc_info(val:elf64_xword):elf64_xword;
begin
 elf64_reloc_info:=val shr 32 shl 32+val and $FFFFFFFF;
end;
function elf_move_symbol(info:dword):dword;
begin
